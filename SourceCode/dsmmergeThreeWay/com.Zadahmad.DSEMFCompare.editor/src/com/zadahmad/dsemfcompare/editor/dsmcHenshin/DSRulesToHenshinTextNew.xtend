	 ...
	 def void setSignatureItems(String henshinFileType)
	 {
	 	...
	 	{	
	 		...
	 		if(getAttrValueByName(n,"isForbidden") == true)
	 			{	action="forbid";	}	 		
	 		else if(getSuperClassName(n).toLowerCase() == "semanticdiff" 
	    ...
	    if(ruleFilePathes!=null && ruleFilePathes.size() > 0){
		    ...
		    try{
			    ...
			    for (rulePath : ruleFilePathes) {			    	
				    ...
				    val resource = resourceSet.getResource(URI.createURI(rulePath), true)
				    for (content : resource.contents) {
						out.print(generatePostProcessorRule(content as EObject));
				    }
				    i++;
				    rulesFilter.get(i)._filtersCount 	= ruleFilterNumber;
				    rulesFilter.get(i)._elementsCount 	= ruleElementNumber;
		...
		rule «getAttrValueByName(it,"name").toString()»(«ruleSignature»)
		{
			graph
			{
			  	«FOR n : allDsDiffRuleNodes»
					...
					«ELSEIF getSuperClassName(n).toLowerCase() == "semanticdiff" 
			  			|| getAttrValueByName(n,"ID_Pattern").toString().contains("-") == true»
...
//----------------------------------------------------------------------
	 ...
	 def void init(EObject ruleN){
	 		...
	 		//ruleNode						=	patternRootNode.eContainer();
	  		allDsDiffRuleNodes				=	DsmComparatorController.getAllNodes(ruleNode, true, true);//	DsmComparatorController.getAllNodes(patternRootNode, true, false);
	 ...
	 {	 
		...
		if(varPostCount ==0 && diffKindVal !="" && getAttrValueByName(n,"diff_kind").toString() !="MODIFY" && getAttrValueByName(n,"diff_kind").toString() !="NON")
		{
	 	...
	 	if(getSuperClassName(n).toLowerCase() == "semanticdiff"){
			  	...
			  	for(ctLeft:leftConstraintCollection){
					...
					if(ctRight.operation.equals("contains")){
						...
						ctRight.conditionText = 
							'("'+ctRight.varInLhs+".trim()."+ctRight.operation+'('+ctRight.varInRhs+'.trim().replace(\'return;\',\'\'))"'+ " == " + "'true'" + ")";
	  	...
	  	try{
			...
			if(ctLeft.operation.equals("contains")){
				...
				ctLeft.conditionText = 
							'("'+ctLeft.varInLhs+".trim()."+ctLeft.operation+'('+ctLeft.varInRhs+'.trim().replace(\'return;\',\'\'))"'+ " == " + "'true'" + ")";
