...
public class DiffClassArray implements IMMExtension {
		...
		if (element instanceof EClass) {
			...
			if (!srcClass.isAbstract()) { 
				...
				EAttribute attrDiff_kind = 
						DsmcHelpers.createEAttribute("diff_kind", diffClass, pack.getEClassifier("DiffKind"), 1, 2, 
				...
				for(EAttribute attr:srcClass.getEAllAttributes())
				{
					if(!attr.isID()) {
						...
						if(attr.getUpperBound() ==1) {	
							DsmcHelpers.createEAttribute(newAttributeName, diffClass, attrType, 0, 2, 
									false, false, true, false);	
						}
						else{	
							...
							DsmcHelpers.createEReference(newAttributeName, diffClass, listClassType, 0, 2, true);
						}						
					}						
				}			
			}
