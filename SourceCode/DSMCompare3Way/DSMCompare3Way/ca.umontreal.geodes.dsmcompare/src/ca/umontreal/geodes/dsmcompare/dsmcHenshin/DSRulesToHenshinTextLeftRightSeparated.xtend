...
class DSRulesToHenshinTextLeftRightSeparated {	 
	...
	def public void runPostRuleGenerator(){
	 	henshinRulesDSDiffPostRulePath	=	dsmCasePath + dsmCaseHenshinRulesFolderName + dsmName + "RulesDSDiffPostRules.henshin_text";
	 	setRulesFilter();
	 	setRulesFilePaths();
	 	generatePostRules(dsmCaseRuleFilePaths,henshinRulesDSDiffPostRulePath)
	}	
	...
	def public void run(boolean isATempFile){
	 ...
	 def void setSignatureItems(String henshinFileType)
	 {
	 	...
	 	{	
	 		...
	 		if(getAttrValueByName(n,"isForbidden") == true)
	 			{	action="forbid";	}	 		
	 		else if(getSuperClassName(n).toLowerCase().contains("semanticdiff") 
		    ...
		    try{
			    ...
			    for (rulePath : ruleFilePathes) {
				    ...
				    for (content : resource.contents) {
						out.print(generateRule(content as EObject, "CPA", author));
				    }
	 ...
	 def generateDSDiff(ArrayList<String> ruleFilePathes, String henshin_textFilePath){	    
	    ...
	    if(ruleFilePathes!=null && ruleFilePathes.size() > 0){
		    ...
		    try{
				...
				val authors = new ArrayList<String>(Arrays.asList("equal", "left", "right")); 
				...
				for (rulePath : ruleFilePathes) {
					...
					for(author:authors){					
						...
						try{
						    ...
						    val resourceSet = new ResourceSetImpl
						    val resource = resourceSet.getResource(URI.createURI(rulePath), true)
						    for (content : resource.contents) {//??? We only need to get the Rule object we don't need a for loop
								out.print(generateRule(content as EObject , "DSDiff", author));						
						    }
			    		...
			    		System.out.println("Error transforming rule: "+rulePath);
			    		System.out.println("Error transforming rule. "+ex.message); 
						}	    		 				
					    allDsDiffRuleNodes.clear();
					    allDsDiffRuleAssocCollection.clear();
				    }
			    ...
			    out.close; 		    
			    		...
			    		System.out.println("Error transforming rules. "+ex.message);  
	    	}		    
	    } 
	  }
	 ...
	 def generatePostRules(ArrayList<String> ruleFilePathes, String henshin_textFilePath){	    
	    ...
	    if(ruleFilePathes!=null && ruleFilePathes.size() > 0){
		    ...
		    try{
			    ...
			    for (rulePath : ruleFilePathes) {			    	
				    ...
				    val resource = resourceSet.getResource(URI.createURI(rulePath), true)
				    for (content : resource.contents) {
						out.print(generatePostProcessorRule(content as EObject, author));
				    }
				    i++;
				    rulesFilter.get(i)._filtersCount 	= ruleFilterNumber;
				    rulesFilter.get(i)._elementsCount 	= ruleElementNumber;
...
ePackageImport  «eClass().getEPackage().getName().replace("CRRule","Diff").replace("2WayRule","Diff")»
	  '''
	  
	  //«init(it)»
	  //«defineWileLoopForRule(getAttrValueByName(it,"name").toString())»
	  
	 def dispatch generateRule(EObject it, String henshinFileType, String author) '''
		«init(eContents().get(0))»
		«defineStarUnitForRule(getAttrValueByName(it,"name").toString(), author)»
		«callStarUnitForRule(getAttrValueByName(it,"name").toString(), author)»
		«setSignatureItems("DSDiff")»
		«findRuleConstraintsAndRuleSignature(author)»

		rule «getAttrValueByName(it,"name").toString()+"_"+author»(«ruleSignature»)
		{
			graph
			{
			  	«FOR n : allDsDiffRuleNodes»
			  		«IF getAttrValueByName(n,"isForbidden") == true »
							«defineNode(n, "forbid", henshinFileType, author)»					
					«ELSEIF getSuperClassName(n).toLowerCase().contains("semanticdiff") 
			  			|| getAttrValueByName(n,"ID_Pattern").toString().contains("-") == true»
							«defineNode(n,"create", henshinFileType, author)»
			      	«ELSE»
							«defineNode(n,"preserve", henshinFileType, author)»			      		
			      	«ENDIF»
			  	«ENDFOR»
			«defineEdges()»
			«IF edges.size() > 0»
			...
			«defineConstraints(author)» «cleanUpVariablesForNextRuleLoop()»
		}
	  '''

/*
			      	«ELSEIF getAttrValueByName(n,"filter") == true »
							«defineNode(n, "filter", henshinFileType, author)» //delete*/
	 //*********************************************** 

	
	 def dispatch generatePostProcessorRule(EObject it, String author) '''
		«init(eContents().get(0))»«getPostRuleNodes()»
		«defineStarUnitForRule(getAttrValueByName(it,"name").toString()+"Post", author)»
		«callStarUnitForRule(getAttrValueByName(it,"name").toString()+"Post", author)» «setVarPostCountToZero()»
	 ...
	 def String defineForLoopPerRule(String ruleName){
	 	var wileLoopForRule = "";
	 	wileLoopForRule += "if("+ruleName+"())then{";
		wileLoopForRule += "for(300){\n\t\t\t";
	    wileLoopForRule +=ruleName+"()"
	    wileLoopForRule +="\n\t\t}}";
	 }
	 
	 def void defineStarUnitForRule(String ruleName, String author){
	 		var starUnitDefinitionForRule = "\n\t"+"unit unit"+ruleName+"_"+author+"Star(){\n\t\t";
		    starUnitDefinitionForRule +=useWhileLoop==true ? defineWileLoopPerRule(ruleName+"_"+author) : defineForLoopPerRule(ruleName+"_"+author);
		    starUnitDefinitionForRule +="\n\t}";
		    DefinitionsStarUnits += starUnitDefinitionForRule;
	 }
	 
	 def void callStarUnitForRule(String ruleName, String author){
	 		 callsStarUnits+= "\n\t\t"+"unit"+ruleName+"_"+author+"Star()";
	 }
	 ...
	 {	 
		...
		if(varPostCount ==0 && diffKindVal !="" && getAttrValueByName(n,"diff_kind").toString() !="MODIFY" && getAttrValueByName(n,"diff_kind").toString() !="NON")
		{
			varPostCount = 1;
			nodeDef += "{";	
	 	...
	 	if(getSuperClassName(n).toLowerCase().contains("semanticdiff")){
	 ...
	 //***********************************************   
	 def String defineNode(EObject n, String actionText, String henshinFileType, String author)
	 {
	 	var nodeDef="\t\t\t\t "+actionText;
	 	if(actionText == "preserve" || actionText == "forbid")
	 		nodeDef+= "\t\t\t"
		...
		val filterValue = DsmcHelpers.getAttrValueByAttrNameNullDefault(n, "filter");
		//set  filter = true
		
		
	 	if(filterValue != null){
	 		if(filterValue.toString().equalsIgnoreCase("true")) {
				ruleFilterNumber+=1;
				//filterAttrDef = "\n\t\t\t\t\t" + actionText + " " + getStringAttrDef(n,"filter");
				filterAttrDef = "\n\t\t\t\t\t" + "preserve" + " filter = false ";
				filterAttrDef += "\n\t\t\t\t\t" + "set" + " filter = true "; //getStringAttrDef(n,"filter");
				nodeDef+= "\t\t\t"	
			}else if(DsmcHelpers.getAttrValueByAttrNameNullDefault(n, "diff_kind") != null){ //To make sure it is a diff class
				filterAttrDef = "\n\t\t\t\t\t" + actionText + " filter = false ";
				nodeDef+= "\t\t\t"
			}	 		
	 	} 
		...
		if(actionText != "create")
	 	...
	 	if(!author.toLowerCase().equals("right")){
	 	...
	 	if(!author.toLowerCase().equals("left")){
	 	...
	 	if(henshinFileType=="DSDiff"){
	 		...
	 		if(!diffKindVal.contains("NON")){// && author.toLowerCase().equals("left")
	 			if(author.toLowerCase().equals("left")){
	 				diffKindLeftDef  = diffKindVal.replace("diff_kind","diff_kind_Left");	 
	 			}else if(author.toLowerCase().equals("right")){
	 	...
	 	var domainspecificdiffNameAttrDef= "";	 	
	 	if(getSuperClassName(n).toLowerCase().contains("semanticdiff") && DsmcHelpers.getAttrValueByAttrNameNullDefault(n,"name").toString()!=""){
	 		domainspecificdiffNameAttrDef = getStringAttrDef(n,"name");
	 	}	 	
	 	
	 	if(ctNodes.size() > 0 || diffKindLeftDef != "" || 
	 		diffKindRightDef !="" || domainspecificdiffNameAttrDef != "" || !filterAttrDef.equals(""))
	 		{//|| attributeNameAttrDef !=""
	 		nodeDef += "{";
	 		
	 		if(domainspecificdiffNameAttrDef!=""){	 			
	 			nodeDef += "\n\t\t\t\t\t" + actionText + " " + domainspecificdiffNameAttrDef;
	 			if(actionText != "forbid")
	 			{
	 				if(author.toLowerCase().equals("left")){
		 				nodeDef += "\n\t\t\t\t\t"  + actionText + " " + "author		=  "+ "\"\\\"LEFT\\\"\""; 
		 			}else if(author.toLowerCase().equals("right")){
		 				nodeDef += "\n\t\t\t\t\t"  + actionText + " " + "author		=  "+ "\"\\\"RIGHT\\\"\"";
		 			}else{
		 				nodeDef += "\n\t\t\t\t\t"  + actionText + " " + "author		=  "+ "\"\\\"EQUAL\\\"\"";
		 			}
	 			}
	 			else
	 			{
	 				var authorObject = DsmcHelpers.getAttrValueByAttrNameNullDefault(n,"author");	 				
	 				if(authorObject != null && authorObject.toString().matches("LEFT|RIGHT|EQUAL")){
	 					nodeDef += "\n\t\t\t\t\t"  + actionText + " " + "author		=  "+ "\"\\\""+authorObject.toString()+"\\\"\"";
	 				}
	 			}
		 	...
		 	if(!filterAttrDef.equals("")){
		 		nodeDef += filterAttrDef;
		 	}
	 ...
	 def String getStringAttrDef(EObject n, String attrName)
	 {
	 	...
	 	if(attr==null){
	 		...
	 		if(!attrName.equals("diff_kind") && attr.getEAttributeType().eClass().getName().toLowerCase()=="eenum")
	 		{	
	 			var id_PatternLhs=getAttrValueByName(n,"ID_Pattern").toString();			
				return  attrName +" = " + "n" + id_PatternLhs + attrName;	 		
			}
			/*else if(attrName.equals("filter")) //&& attr.getEAttributeType().eClass().getName().toLowerCase()=="eboolean"){	
			{	 						
	 ...
	 def void defineEdges(){
	  	...
	  	for(EReferenceType ERefC: getAllAssocCollection()){
	  		if(allDsDiffRuleNodes.contains(ERefC.source) && allDsDiffRuleNodes.contains(ERefC.target))
	  		{
		  		...
		  		else if(ERefC.source.eClass().getName().toLowerCase().contains("semanticdiff")  
		  			||	ERefC.target.eClass().getName().toLowerCase().contains("semanticdiff")
	 ...
	 def findRuleConstraintsAndRuleSignature(String author){
	 	if(ruleNode !== null){
	 		leftConstraintCollection = new ArrayList<ConstraintType>();
			...
			if(consts != ""){
				...
				for(var i=1;i<cs.size();i++){
			  	...
			  	if(!author.trim().toLowerCase().equals("right")){
				  	for(c:leftConstraintCollection){
				  		addParamToRuleSignature(c.ruleInLhs, author);
				  		addParamToRuleSignature(c.ruleInRhs, author);			
			 		}
		 		}	

				if(!author.trim().toLowerCase().equals("left")){
	 ...
	 def Boolean addParamToRuleSignature(String param, String author){
	 	if(param!=null && param!="" && !ruleSignature.contains(param) && 
	 		(author.trim().toLowerCase().equals("equal") || (author.trim().toLowerCase().equals("left") && !param.toLowerCase().endsWith("_right"))
	 			|| (author.trim().toLowerCase().equals("right") && !param.toLowerCase().endsWith("_left"))))
	 	{	 		 	 		
	 			
			if(ruleSignature!="" && ruleSignature!=null)
			 	{ 
			 		if(param.trim().endsWith(":")){
			 			ruleSignature += "," + param + "EString";				 			
			 		}
			 		else{
			 			ruleSignature += "," + param;			 			
			 		}
	 ...
	 def defineConstraints(String author){
	  	ruleSignature 				=   "";
	  	totalConstraintDefinition 	= 	"";		
		 	...
		 	/*for(ct:leftConstraintCollection){
	  		...
	  		&&  totalRightconstraintDefinition!="" && totalRightconstraintDefinition!=null)
	  			{ 
	  				...
	  				totalConstraintDefinition = "conditions [(" +  totalLeftconstraintDefinition 
	  				+ ") AND (" + totalRightconstraintDefinition +")]"; //OR
	 ...
	 def ConstraintType constraintDsDiffToConstraintType(String constraintDsDiff){
	  	try{
			...
			else if(ctLeft.operation.equals("contains2")){
						//"\"MODIFY\""
						ctLeft.conditionText = 
