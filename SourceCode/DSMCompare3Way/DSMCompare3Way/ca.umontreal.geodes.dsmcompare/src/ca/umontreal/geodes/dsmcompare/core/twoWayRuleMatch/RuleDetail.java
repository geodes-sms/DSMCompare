package ca.umontreal.geodes.dsmcompare.core.twoWayRuleMatch;
...
public class RuleDetail {
	...
	public void init(String RulePath, String RuleMM_eNSURI, Object RuleMM_eIN) {
		 ruleFilePath 	= RulePath;
		 ruleMM_eNSURI	= RuleMM_eNSURI;
		 ruleMM_eIN		= RuleMM_eIN;
		 //rootDSDiff		= RootDSDiff;
		 ResourceSet rsetRule	= DsmcHelpers.setResourceSet(ruleFilePath,ruleMM_eNSURI, ruleMM_eIN);	
		 Resource rsourceRule	= rsetRule.getResource(URI.createFileURI(ruleFilePath) , true);
		 ruleNode 		= rsourceRule.getContents().get(0);		
		 ruleName		= (String) DsmcHelpers.getAttrValueByAttrName(ruleNode, "name");
		 ...
		 */		 
		 semanticName	= (String) DsmcHelpers.getAttrValueByAttrName(semanticNode, "name");
	}
