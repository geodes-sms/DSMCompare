package ca.umontreal.geodes.dsmcompare.core.RuleEngine;
...
import ca.umontreal.geodes.dsmcompare.mmextension.utils.DsmcHelpers.SrcRefTrgt;
...
public class DsDiffModelToDsDiffRule {
	...
	void transform(String dsDiffModelPath, String dsDiffRulePath) {
		if(dsDiffRulePath!=""){
			try {	
				ResourceSet resourceSet		= DsmcHelpers.getResourceSetXMI(dsDiffModelPath,diffMM_eNSURI,diffMM2Way_eIN);
				Resource 	resourceDiffModel 	
											= DsmcHelpers.getResource(dsDiffModelPath, resourceSet);
			    ...
			    Resource resourceRule 		= DsmcHelpers.createAndAddResource(dsDiffRulePath, new String[] {"xmi"}, resourceSetRule);//"gast", 			    
			    ...
			    EAttribute	attrName 		= DsmcHelpers.getEAttributeByName(ruleEObject, "name");
				EReference  refPattern		= DsmcHelpers.getEReferenceByNameNullDefault(ruleEObject, "pattern");
						...
						if(r instanceof  EReference){			
						  try{
						  		...
						  		for (EObject child:children){
									...
									DsmcHelpers.SetEFeature(ruleEOBj, refInRuleParent, ruleEObjChild);
											...
											if((attr.getName().equals("diff_kind_Left") 
												...
												EAttribute attrChild=ruleEObjChild.eClass().getEAllAttributes().stream().filter(
														a->a.getName().equals(attr.getName())).collect(
																Collectors.toList()).get(0);
												DsmcHelpers.SetEFeature(ruleEObjChild, attrChild, child.eGet(attr));
											}											
										}catch(Exception ex) {
				...
				EObject ruleRoot = DsmcHelpers.getERefTargetByERefName(ruleObject,"pattern").get(0);				
				EReference refTypeDSD = DsmcHelpers.getEReferenceByNameNullDefault(ruleRoot,"domainspecificdiffs");
				...
				EAttribute	dsdAttrName = DsmcHelpers.getEAttributeByName(dsdEObject, "name");
				DsmcHelpers.SetEFeature(dsdEObject, dsdAttrName,ruleName);				
				DsmcHelpers.SetEFeature(ruleRoot, refTypeDSD,dsdEObject);
				...
				while(!q.isEmpty()) {
					...
					if(refSecondSection.length()>0) {
						...
						refTypedsdRef = DsmcHelpers.getEReferenceByNameNullDefault(dsdEObject,dsdRefName);
					...
					if(refTypedsdRef != null) {
						if(refTypedsdRef.getUpperBound()==1) {
							...
							if(objT != null && !objT.equals(ruleEOBj))
								DsmcHelpers.SetEFeature(dsdEObject, refTypedsdRef,ruleEOBj);
	...
	public void RemoveFilteredReferencesFromDSDObjet() {
		...
		if(dsDiffRulePath!=""){
			try {				
				ResourceSet resourceSet			= DsmcHelpers.getResourceSetXMI(dsDiffRulePath,ruleMM2Way_eNSURI,ruleMM2Way_eIN);
				Resource 	resourceRule 		= DsmcHelpers.getResource(dsDiffRulePath, resourceSet);
				EObject 	ruleObject 			= (EObject)resourceRule.getContents().get(0);	
				ArrayList<EObject> rootEObject	= DsmcHelpers.getERefTargetByERefName(ruleObject,"pattern");
				ArrayList<EObject> dsdEObjects	= DsmcHelpers.getERefTargetByERefName(rootEObject.get(0),"domainspecificdiffs");
						...
						if(r instanceof  EReference){			
						  try{
							  	...
							  	ArrayList<EObject>  removeList = new ArrayList<EObject>();
					  		  	if(children != null && children.size()>0){
							  		for (EObject child:children){
							  			Object filter = DsmcHelpers.getAttrValueByAttrNameNullDefault(child,"filter");
