...
public class dsvcshenshin {
	...
	public void runCpaForAttributeChange() {
		...
		ArrayList<Rule> attributeChangeFineRules = new ArrayList<Rule>();
		
		for(Unit u:usCPA) {
			rulesSemanticCPA.add((Rule) u);
		}
		for(Unit u:usFines) {
			attributeChangeFineRules.add((Rule) u);
		}
			...
			for(org.eclipse.emf.henshin.model.Rule ruleFirst:rulesSemanticCPA) {
				...
				for(org.eclipse.emf.henshin.model.Rule ruleSecond:attributeChangeFineRules) {
					try {
						...
						System.out.print("\nError: "+ruleFirst.getName()+" - "+ruleSecond.getName());
