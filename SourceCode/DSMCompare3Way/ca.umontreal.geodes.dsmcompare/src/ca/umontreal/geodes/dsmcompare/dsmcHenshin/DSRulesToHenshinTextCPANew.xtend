...
class DSRulesToHenshinTextCPANew {	 
	...
	def public void run(boolean isATempFile){
	 ...
	 def public setRulesFilter(){
		for(String ruleFileName:dsmCaseRuleFileNames){
			rulesFilter.add(new RuleAttributes(ruleFileName.substring(0,ruleFileName.length-4), -1,-1, -1));
		}	  		 	
	 }
			...
			graph
			{
			  	«FOR n : allDsDiffRuleNodes»
					...
					«ELSEIF getSuperClassName(n).toLowerCase().contains("semanticdiff") 
			  			|| getAttrValueByName(n,"diff_kind").toString().contains("ADD") == true
			  			|| getAttrValueByName(n,"diff_kind").toString().contains("MODIFY") == true
			  			|| getAttrValueByName(n,"ID_Pattern").toString().contains("-") == true»
							«defineNode(n,"create", henshinFileType)»
			      	«ELSEIF getAttrValueByName(n,"diff_kind").toString().contains("DELETE") == true»
...
//----------------------------------------------------------------------
	 ...
	 def void defineEdges(){
	  	...
	  	for(EReferenceType ERefC: getAllAssocCollection()){
	  		...
	  		{
		  		if(getAttrValueByName(ERefC.source,"isForbidden") == true	  
		  			|| getAttrValueByName(ERefC.target,"isForbidden") == true)
		  		{
		  			var dontCreate = false;
		  			if(
		  				ERefC.target.eClass().getName().toLowerCase().contains("semanticdiff")
		  			||  getAttrValueByName(ERefC.target,"diff_kind").toString().contains("ADD") == true
		  			||  getAttrValueByName(ERefC.target,"diff_kind").toString().contains("MODIFY") == true
					...
					edges.add(AssocDef);
		  			...
		  			(ERefC.source.eClass().getName().toLowerCase().contains("semanticdiff") 
		  			||  getAttrValueByName(ERefC.source,"diff_kind").toString().contains("ADD") == true
		  			||  getAttrValueByName(ERefC.source,"diff_kind").toString().contains("MODIFY") == true
		  		...
		  		{	
		  			var dontCreate = false;
		  			if(getAttrValueByName(ERefC.source,"diff_kind").toString().contains("DELETE") == true 
		  			||getAttrValueByName(ERefC.target,"diff_kind").toString().contains("DELETE") == true)
		  				dontCreate = true;
		  		...
		  		else if(getAttrValueByName(ERefC.source,"diff_kind").toString().contains("DELETE") == true
		  			||  getAttrValueByName(ERefC.target,"diff_kind").toString().contains("DELETE") == true)
		  		{
		  			var dontCreate = false;
		  			if(ERefC.target.eClass().getName().toLowerCase().contains("semanticdiff")
		  			||  getAttrValueByName(ERefC.target,"diff_kind").toString().contains("ADD") == true
		  			||  getAttrValueByName(ERefC.target,"diff_kind").toString().contains("MODIFY") == true
		  			||  (getAttrValueByName(ERefC.target,"ID_Pattern").toString().contains("-") == true))
		  				dontCreate = true;
