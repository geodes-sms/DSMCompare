...
public final class ProcessComparisonResult {
	...
	public static  ComparisonResult processThreeWayComparison(Comparison comparison3Way, String resultFilePath) throws FileNotFoundException {
		...
		Long 
		realConflictsCount=comparison.getConflicts().stream().filter(c->c.getKind() == ConflictKind.REAL).count(), 
		pSeudoConflictsCount=comparison.getConflicts().stream().filter(c->c.getKind() == ConflictKind.PSEUDO).count();
				
		stream.println("Matches count 	:	" + matchesCount.toString());
		stream.println("Diffs count 	:	" +  diffsCount.toString());
		stream.println("Conflicts count (REAL) :	" + realConflictsCount.toString());
		stream.println("Conflicts count (PSEUDO) :	" + pSeudoConflictsCount.toString());
	...
	public static  void processThreeWayComparison(ComparisonResult comparisonResult, String resultFilePath) throws FileNotFoundException {
		//Comparison comparison3Way
		PrintStream stream = initStream(resultFilePath);
		comparison = comparisonResult.comparison3Way;
	...
	private static  void printConflict(Conflict conflict, PrintStream stream, ComparisonResult	comparisonResult) {
		...
		for (Diff diff : attChanges) {
			...
			processDiff(diff, stream, comparisonResult);
				...
				stream.println("Exception (processDiff method) :" + ex.toString());
				System.out.println("Exception (processDiff method) :" + ex.toString());
			}
		}
	...
	private static  void processDiff(Diff diff, PrintStream stream, ComparisonResult	comparisonResult) {
		...
		if (diff instanceof ReferenceChange) {
			...
			if (valueName != null) {
				...
				if(fineDiffReference.reference.isContainment() //|| fineDiffReference.diffKind == DifferenceKind.MOVE //For multi-valued move, the delete from part didn't work
						) {
