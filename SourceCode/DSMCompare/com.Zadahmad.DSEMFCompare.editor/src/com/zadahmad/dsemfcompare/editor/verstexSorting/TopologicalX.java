...
public class TopologicalX {
    ...
    public List<ArrayList<Integer>> getTopologicalOrderSets(Digraph G, List<Integer> itemsToRemove){
        // indegrees of remaining vertices
        int[] indegree = new int[G.V()];
        for (int v = 0; v < G.V(); v++) {
            indegree[v] = G.indegree(v);
        ...
        ranks = new int[G.V()]; 
        order = new Queue<Integer>();
        List<ArrayList<Integer>> orderedSets = new ArrayList<ArrayList<Integer>>();
        int count = 0;
        ...
        while (!queue.isEmpty()) {
            ...
            order.enqueue(v);
            ranks[v] = count++;
            orderedSet = new ArrayList<Integer>();
            orderedSets.add(orderedSet);
            for (int w : G.adj(v)) {
                indegree[w]--;
                if (indegree[w] == 0) {
                	queue.enqueue(w);
                	if(!itemsToRemove.contains(w)) {
                		orderedSet.add(w);  
                	}                		         	
                }
            }
