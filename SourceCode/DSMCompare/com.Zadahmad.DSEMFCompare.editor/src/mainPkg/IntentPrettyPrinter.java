...
public final class IntentPrettyPrinter {
	...
	public static  void printDifferences(EList<Diff> diffs, PrintStream stream) {
		final Iterable<ReferenceChange> refChanges = Iterables.filter(diffs,
				ReferenceChange.class);
		stream.println(diffs.size());
		stream.println("REFERENCE CHANGES");
		for (Diff diff : refChanges) {
			printDiff(diff, stream);
		}
		...
		stream.println("ATTRIBUTE CHANGES");
		final Iterable<AttributeChange> attChanges = Iterables.filter(diffs,
				AttributeChange.class);
		for (Diff diff : attChanges) {
			printDiff(diff, stream);
		}
